...
func getIntrospectHandler(srv *server.Server) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			slog.Warn("introspect: failed to get client", "client_id", username, "error", err)
		...
		if err != nil {
			slog.Warn("introspect: invalid or expired token", "error", err)
		...
		slog.Info("introspect: successful introspection", "client_id", cli.GetID(), "token_user_id", ti.GetUserID())
